"""
MIT License
Copyright (c) 2024 TheHamkerCat
"""

from os import remove
from os.path import isfile
from time import ctime, time
from traceback import format_exc

from pyrogram import filters
from pyrogram.types import Message

from wbb import SUDOERS, USERBOT_PREFIX, app2, eor
from wbb.core.sections import section
from wbb.core.tasks import add_task, rm_task
from wbb.utils.downloader import download


@app2.on_message(
    SUDOERS
    & filters.command("download", prefixes=USERBOT_PREFIX)
    & ~filters.forwarded
    & ~filters.via_bot
)
async def download_func(_, message: Message):
    reply = message.reply_to_message
    start = time()

    body = {
        "دەستپێکردن": ctime(start),
    }
    m = await eor(
        message,
        text=section("خەریکی داونلۆدکردنم", body),
    )

    if reply:
        task, task_id = await add_task(
            reply.download,
            task_name="Downloader",
        )

        body["ئایدی ئەرک (Task ID)"] = task_id
        await eor(
            m,
            text=section("خەریکی داونلۆدکردنم", body),
        )

        await task
        await rm_task(task_id)

        file = task.result()

        elapsed = int(time() - start)
        body["فایل"]: file.split("/")[-1]
        body["ماوەی خایەنراو"]: f"{elapsed} چرکە"

        return await eor(m, text=section("داونلۆد کرا ✅", body))

    text = message.text
    if len(text.split()) < 2:
        return await eor(m, text="زانیارییەکان هەڵەن (لینکت نەداوە)")

    url = text.split(None, 1)[1]

    try:
        task, task_id = await download(url)

        body["ئایدی ئەرک (Task ID)"] = task_id
        await eor(
            m,
            text=section("خەریکی داونلۆدکردنم", body),
        )

        await task
        file = task.result()
        await rm_task(task_id)

    except Exception as e:
        e = format_exc()
        e = e.splitlines()[-1]
        return await eor(m, text=f"**هەڵە:** `{str(e)}`")

    elapsed = int(time() - start)
    body["فایل"] = file.split("/")[-1]
    body["ماوەی خایەنراو"] = f"{elapsed} چرکە"

    await eor(m, text=section("داونلۆد کرا ✅", body))


@app2.on_message(
    SUDOERS
    & filters.command("upload", prefixes=USERBOT_PREFIX)
    & ~filters.forwarded
    & ~filters.via_bot
)
async def upload_func(_, message: Message):
    if len(message.text.split()) != 2:
        return await eor(message, text="زانیارییەکان هەڵەن (ناونیشانی فایلت نەداوە)")

    url_or_path = message.text.split(None, 1)[1]

    start = time()

    body = {
        "دەستپێکردن": ctime(start),
    }

    m = await eor(message, text=section("خەریکی ئەپڵۆدکردنم", body))

    async def upload_file(path: str):
        task, task_id = await add_task(
            message.reply_document,
            "Uploader",
            path,
        )

        body["ئایدی ئەرک (Task ID)"] = task_id
        await eor(
            m,
            text=section("خەریکی ئەپڵۆدکردنم", body),
        )

        await task
        await rm_task(task_id)

        elapsed = int(time() - start)
        body["ماوەی خایەنراو"] = f"{elapsed} چرکە"

        return await eor(m, text=section("ئەپڵۆد کرا ✅", body))

    try:
        if isfile(url_or_path):
            return await upload_file(url_or_path)

        task, task_id = await download(url_or_path)

        body["ئایدی ئەرک (Task ID)"] = task_id
        await eor(
            m,
            text=section("خەریکی داونلۆدکردنی کاتیی فایلم", body),
        )
        await task
        file = task.result()
        await rm_task(task_id)

        await upload_file(file)
        remove(file)
    except Exception as e:
        e = format_exc()
        e = e.splitlines()[-1]
        return await eor(m, text=f"**هەڵە:** `{str(e)}`")
